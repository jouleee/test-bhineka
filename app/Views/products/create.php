...
<?= $this->section('page_title') ?>Tambah Produk Baru<?= $this->endSection() ?>
...
<div class="card" style="max-width: 600px; margin: 0 auto;">
    ...
    <form action="<?= base_url('products/store') ?>" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="code" id="code" class="form-control" value="<?= old('code') ?>" placeholder="Contoh: PR04" required autofocus>
        </div>

        <div class="form-group">
            <label class="form-label" for="name">Nama Produk / Barang</label>
            <input type="text" name="name" id="name" class="form-control" value="<?= old('name') ?>" placeholder="Contoh: Ban Dalam" required>
        </div>

        <div class="form-row">
            <div class="form-group">
                <label class="form-label" for="unit">Satuan (Unit)</label>
                <input type="text" name="unit" id="unit" class="form-control" value="<?= old('unit') ?>" placeholder="Contoh: Pcs, Dus, Liter, Unit" required>
            </div>
